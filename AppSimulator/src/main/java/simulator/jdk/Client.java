...
public class Client implements Runnable
{
    ...
    public void run()
    {
        do
        {
            ...
            switch (rww)
            {
                case READ:
                    ...
                    if (currentApp != null)
                    {
                        ...
                        if ((match == 0 && ThreadLocalRandom.current().nextInt(100000) == 5000 && giveFeedback) || (!runInfinitely && giveFeedback))
                ...
                case WRITE:
                    ...
                    if ((currentApp != null && ThreadLocalRandom.current().nextInt(1000000) == 5000 && giveFeedback) || (!runInfinitely && giveFeedback))
            ...
            COUNT++;
        } while (runInfinitely && COUNT < 1000);
    }
    ...
    private String doWrite(App currentApp)
    {
        ...
        if (rand == 0)
        {
            ...
            if (appMenu.apps.putIfAbsent(name, new App(name)) == null)
        ...
        {
            if (appMenu.apps.remove(currentApp.toString()) != null)
        ...
        {
            if (appMenu.apps.remove(currentApp) != null)
            {
                currentApp.renameApp(StringConstants.getName());
                appMenu.apps.put(currentApp.toString(), currentApp);
                response = name + " renamed an app to " + currentApp.toString();
            }
            else
                response = currentApp.toString() + " has already been renamed or removed!";
        }
        return response;
    }
}
